# 3bebbd17-ade8-4e22-bb46-02b7a90f06fe implementation handoff

This archive is the source of truth for turning the design into production code. Start from `english-cockpit-os.html`, then preserve the visual system, responsive behavior, and interactions found in the exported files.
...
## Source map
- Primary entry: `english-cockpit-os.html`
...
- A single self-contained `english-cockpit-os.html` is acceptable only when the export truly contains one user-facing screen and its CSS/JS are structured enough to extract tokens, components, states, and behavior.
...
1. Open `english-cockpit-os.html` and `DESIGN-MANIFEST.json`; identify every screen file, launcher/overview file, app module, and interaction before coding.
...
1. Inspect `english-cockpit-os.html` and `DESIGN-MANIFEST.json` first and identify reusable components before coding.
