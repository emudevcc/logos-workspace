// Shared list/detail/delete/star logic for saved Bíblia studies.
//
// Both the Historial and Favoritos views are the same UI — fetch a list of
// saved studies, open one, delete one, toggle its favorite flag — differing
// only in the fetch path (all studies vs. favorites-only) and the empty-state
// wording. This module owns that logic once so the star toggle cannot behave
 ...
 * @param {{ fetchPath: string, emptyMessage: string }} options
 */
export function initSavedStudyList(slot, { fetchPath, emptyMessage }) {
  ...
  async function loadList() {
    ...
    try {
      const summaries = await apiGet(fetchPath);
      if (!summaries.length) {
        listEl.append(h("p", { class: "muted", text: emptyMessage }));
        return;
      }
  ...
  function buildRow(summary) {
    const starBtn = h("button", {
      type: "button",
      class: "chip",
      // Filled star when favorited, outline when not — the same pair the
      // Favoritos tab icon uses, so the meaning is consistent across the app.
      text: summary.is_favorite ? "★" : "☆",
      title: summary.is_favorite ? "Quitar de favoritos" : "Añadir a favoritos",
      "aria-pressed": summary.is_favorite ? "true" : "false",
      onclick: async () => {
        try {
          // Exactly { favorite: bool }: FavoriteRequest is extra="forbid".
          await apiPost(`/api/bible/studies/${summary.id}/favorite`, {
    ...
    return h(
      ...
      h("div", { class: "chips" }, starBtn, openBtn, deleteBtn),
    );
  }
